...
contract Pair is CCIPReceiver {
    ...
    function removeLiquidityInUSD(uint256 usdAmount) external {
     ...
     // @dev View function to get the current reserves of the pool and their equivalent USD value.
     // @return (numberoftokens0, usdvalueoftokens0, numberoftokens1, usdvalueoftokens1) Reserves of token0 in amount, token0 in USD, reserves of token1 in amount, and token1 in USD.
    ...
    function getReserves() public view returns (uint256 numberoftokens0, uint256 usdvalueoftokens0, uint256 numberoftokens1, uint256 usdvalueoftokens1) {
        uint256 reserve0InUSD = reserve0 * uint256(getPrice(token0)) / 10**18;
        uint256 reserve1InUSD = reserve1 * uint256(getPrice(token1)) / 10**18;
     ...
     */
    function getPrice(address inputToken) internal view returns (int) {
     ...
     */
    function getPriceRatio() internal view returns (int) {
...
/**
 * @dev View function to get the price of token1 (LINK) in terms of token0 (USDC) using the AMM formula.
 * @return uint256 Price of token1 in terms of token0.
 */
function getDEXPrice() public view returns (uint256) {
    require(reserve0 > 0 && reserve1 > 0, "Reserves are not set");
    return (reserve0 * 10**18) / reserve1; // Price of token1 (LINK) in terms of token0 (USDC)
}


/**
 * @dev View function to get the current market price ratio of token1 (LINK) to token0 (USDC) using Chainlink price feeds.
 * @return uint256 Market price ratio of token1 to token0.
 */
function getMarketPrice() public view returns (uint256) {
    (,int price0,,,) = priceFeed0.latestRoundData(); // Price of USDC in USD
    (,int price1,,,) = priceFeed1.latestRoundData(); // Price of LINK in USD
    return (uint256(price1) * 10**18) / uint256(price0); // Market price of token1 (LINK) in terms of token0 (USDC)
}
